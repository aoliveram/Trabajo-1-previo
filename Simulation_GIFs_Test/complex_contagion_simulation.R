...
T_type <- c("abs", "frac")[2]  # Set the threshold type to "abs" (absolute)
...
image_write(animation, "complex_simulation.gif")
...
#-------------- pruebas --------- 
thresholds[seed]
seeds <- as.numeric(neighbors(graph, seed, mode = "total")) # inside get_complex
unique(seeds) # inside get_complex
num_seeds_i <- num_seeds_to_add[seed]
seeds_to_add <- numeric(num_seeds_i)
if (num_seeds_i > 0) {
  # Get the neighbors of the seed in the graph
  seeds <- as.numeric(neighbors(graph, seed, mode = "total"))
