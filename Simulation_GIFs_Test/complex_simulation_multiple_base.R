...
plot_network <- function(g, activated, N, layout, step, plot_dir) {
  ...
  plot(g, layout = layout, vertex.color = vertex_color,
       vertex.size = 2.5, edge.width = 0.3, vertex.label.cex = 0.5, edge.arrow.size = 0, main = paste("Step", step))
  dev.off()
}
...
get_complex_plot <- function(seed1, seed2, N, g, gmat, thresholds1, thresholds2, num_seeds_to_add1, num_seeds_to_add2, plot, plot_dir) {
  ...
  while (spread) {
    ...
    print(step)
    if (plot) {plot_network(g, activated, N, layout, step, plot_dir)}
  ...
  plot_network(g, activated, N, layout, step, plot_dir)
...
image_write(animation, "complex_TEST_3.gif")
