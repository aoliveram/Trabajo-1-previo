# Only with seeds selected by alpha's value, and requirement of alpha<0.9 and >0.1
...
plot_network <- function(g, activated, N, layout, step, plot_dir, alpha_1, alpha_2) {
  ...
  plot(g, layout = layout, vertex.color = vertex_color,
       ...
       main = sprintf("Alpha Seed 1: %.2f | Alpha Seed 2: %.2f | Step: %d", alpha_1, alpha_2, step))
  dev.off()
}

# Función para simular la difusión compleja y guardar los plots
get_complex_plot <- function(seed1, seed2, N, g, gmat, thresholds1, thresholds2, num_seeds_to_add1, num_seeds_to_add2, plot, plot_dir, alpha_1, alpha_2) {
  ...
  while (spread) {
    ...
    print(step)
    if (plot) {plot_network(g, activated, N, layout, step, plot_dir, alpha_1, alpha_2)}
  ...
  plot_network(g, activated, N, layout, step, plot_dir, alpha_1, alpha_2)
...
# Run the model
plot <- TRUE
results <- get_complex_plot(seed1, seed2, N, graph, gmat, thresholds1, thresholds2, 
...
image_write(animation, "complex_base_OK_04.gif")
