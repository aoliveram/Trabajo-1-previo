...
sweep_homoph_parameter <- function(seed, homoph_values, N, graph, thresholds, num_seeds_to_add, plot, file_dir, alpha_values, graph_file, centrality, T_str, i) {
  ...
  parameter_sweep_homoph_folder <- sprintf("Simulation_Stats_TFrac_3/%s/1inf_N%s_hom_%s_%s_by_%s_alpha_%s_%s_by_%s_T%s",  
    ...
    for (j in seq_along(homoph_values)) {
      ...
      if (plot) {
        ...
        for (k in 1:results$num_steps) {
          img <- image_read(sprintf("Simulation_Stats_TFrac_3/%s/step_%03d.png", file_dir, k))
          images <- c(images, img)
        }
        image_write(image_animate(image_join(images), fps = 1), sprintf("Simulation_Stats_TFrac_3/%s/1inf_%02d.gif", parameter_sweep_homoph_folder, which(alpha_1 == alpha_values)))
      }
    }
...
#graphs <- generate_small_world_networks(N, num_edges, p) #OK - Small-World
#graphs <- generate_scale_free_networks(N, m)             #OK - Scale-Free
#graphs <- generate_erdos_renyi_networks(N, num_edges)    #OK - Erdos-Renyi
#real_sm_net_homoph <- TRUE     #(uncomment for SDA)      #OK - Small-World-SDA 137.85 seg.

graph_file <- 'erdos-renyi-SDA'
...
#saveRDS(failed_nodes, file = sprintf("Simulation_Stats_TFrac_3/%s/failed_nodes.rds", graph_file))
...
# Directorio de datos
data_dir <- sprintf("Simulation_Stats_TFrac_3/%s/1inf_N%s_hom_0.00_0.30_by_0.06_alpha_0.00_1.00_by_0.02_T%s", graph_file, N, T_str)
...
output_file_pdf <- sprintf("Simulation_Stats_TFrac_3/%s/plot_T%s.pdf", graph_file, T_str)
