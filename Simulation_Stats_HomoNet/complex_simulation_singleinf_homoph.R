...
sweep_homoph_parameter <- function(seed, homoph_values, N, graph, thresholds, num_seeds_to_add, plot, file_dir, alpha_values, graph_file, centrality) {
  ...
  parameter_sweep_homoph_folder <- sprintf("Simulation_Stats_HomoNet/%s/1inf_N400_hom_%s_%s_by_%s_alpha_%s_%s_by_%s", 
    ...
    for (j in seq_along(homoph_values)) {
      ...
      if (plot) {
        ...
        for (k in 1:results$num_steps) {
          img <- image_read(sprintf("Simulation_Stats_HomoNet/%s/step_%03d.png", file_dir, k))
          images <- c(images, img)
        }
        image_write(image_animate(image_join(images), fps = 1), sprintf("Simulation_Stats_HomoNet/%s/1inf_%02d.gif", parameter_sweep_homoph_folder, which(alpha_1 == alpha_values)))
      }
    }
...
# 'Homofilicar' una red
create_homophilic_network <- function(graph, similarity_level) {
  
  similarity_func <- function(alpha1, alpha2) {return(1 - abs(alpha1 - alpha2))}
  
  # Obtenemos alpha's de los nodos
  alphas <- V(graph)$alpha
  edges_to_rewire <- c()
  ...
  for (e in E(graph)) {
    ...
    if (similarity_func(alphas[u], alphas[v]) < similarity_level) {
      edges_to_rewire <- c(edges_to_rewire, e)
    }
  }
  
  graph <- delete_edges(graph, edges_to_rewire)
  
  for (e in edges_to_rewire) {
    repeat {
      new_edge <- sample(V(graph), 2)
      if (!are_adjacent(graph, new_edge[1], new_edge[2])) {
        if (similarity_func(alphas[new_edge[1]], alphas[new_edge[2]]) >= similarity_level) {
          graph <- add_edges(graph, new_edge)
          break
        }
      }
...
graph <- create_homophilic_network(graph, similarity_level)
...
graph_file <- 'erdos-renyi-homophilic'
...
T_type <- c("abs", "frac")[1]
top_nodes_all <- calculate_top_nodes(graph, T_dist, T_type)

# Top nodes centralities
top_PLci <- order(top_nodes_all$PLci, decreasing = TRUE)[1:25]
top_degree <- order(top_nodes_all$degree, decreasing = TRUE)[1:25]
top_betweenness <- order(top_nodes_all$betweenness, decreasing = TRUE)[1:25]
top_eigen <- order(top_nodes_all$eigen, decreasing = TRUE)[1:25]
top_percolation <- order(top_nodes_all$percolation, decreasing = TRUE)[1:25]
...
saveRDS(failed_nodes_list, file = sprintf("Simulation_Stats_HomoNet/%s/failed_nodes_list.rds", graph_file))
...
data_dir <- sprintf("Simulation_Stats_HomoNet/%s/1inf_N400_hom_0.00_0.50_by_0.01_alpha_0.00_1.00_by_0.01", graph_file)
...
#plotly::save_image(fig, "simple_plot.png", format = "png")

failed_nodes_list <- readRDS(sprintf("Simulation_Stats_HomoNet/%s/failed_nodes_list.rds", graph_file))
...
for (i in 1:length(centralities)) {
  ...
  #if (centrality %in% colnames(failed_nodes_list)) {
  #  seeds_tried <- length(failed_nodes_list[[centrality]]) + 1
  #  set_title = sprintf("Graph: %s. A: uniform. N= %s. Seed: %s, %s best by: %s", graph_file, N, node_seed, seeds_tried, centrality)
  #}
  seeds_tried <- length(failed_nodes_list[[centrality]]) + 1
  ...
  fig <- plot_ly(
    ...
    layout( 
      title = sprintf("Graph: %s. T=2. N= %s. Seed: %s, %s best by: %s", graph_file, N, node_seed, seeds_tried, centrality),
      scene = list(
        aspectmode = "cube",
        xaxis = list(title = 'h'),
  ...
  output_file <- sprintf("Simulation_Stats_HomoNet/%s/1inf_N400_hom_0.00_0.50_by_0.01_alpha_0.00_1.00_by_0.01_%s.html", graph_file, centrality)
  saveWidget(fig, output_file)
  
  output_file_png <- sprintf("Simulation_Stats_HomoNet/%s/1inf_N400_hom_0.00_0.50_by_0.01_alpha_0.00_1.00_by_0.01_%s.png", graph_file, centrality)
  save_image(fig, output_file_png, format = "png")
}
