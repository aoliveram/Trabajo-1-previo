...
calculate_neighbor_heterogeneity <- function(graph) {
  sapply(V(graph), function(node) {
    neighbors_alpha <- V(graph)$alpha[neighbors(graph, node)]
    if (length(neighbors_alpha) > 1) {
      return(sd(neighbors_alpha))
    } else {
      return(NA)
    }
  })
...
heterogeneity_before <- calculate_neighbor_heterogeneity(graph)

similarity_level <- 0.8
homo_graph <- create_homophilic_network(graph, similarity_level)
heterogeneity_after <- calculate_neighbor_heterogeneity(homo_graph)

# Comparar la heterogeneidad antes y después
...
ggplot(comparison, aes(x = heterogeneity_before, y = heterogeneity_after)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Comparación de la Heterogeneidad de los Vecinos",
       x = "Heterogeneidad Antes",
       y = "Heterogeneidad Después") +
  xlim(0.0, 0.5) +
  ylim(0.0, 0.5) +
  theme_minimal()
...
ggplot(data.frame(degree = degree(graph)), aes(x = degree)) +
  ...
  labs(title = "Distribución de Grado Antes",
       x = "Grado",
       y = "Frecuencia") +
  geom_vline(xintercept = mean(degree(graph)), linetype = "dashed", color = "red") +
  theme_minimal()
ggplot(data.frame(degree = degree(homo_graph)), aes(x = degree)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black") +
  labs(title = "Distribución de Grado Después",
       x = "Grado",
       y = "Frecuencia") +
  geom_vline(xintercept = mean(degree(homo_graph)), linetype = "dashed", color = "red") +
  theme_minimal()
...
print(edge_density(homophilic_graph))

if (plot) {
  layout <- layout_on_grid(homophilic_graph)
  plot(homophilic_graph, layout = layout, vertex.size = 3, edge.width = 0.5, vertex.label.cex = 0.5, 
       edge.color = "gray", edge.arrow.size = 0, vertex.label = round(V(homophilic_graph)$alpha, 2))
}
...
sweep_homoph_parameter <- function(seed, homoph_values, N, graph, thresholds, num_seeds_to_add, plot, file_dir, alpha_values, graph_file, centrality) {
  ...
  parameter_sweep_homoph_folder <- sprintf("Simulation_Stats_Homophilic_Network/%s/1inf_hom_%s_%s_by_%s_alpha_%s_%s_by_%s", 
...
file_dir <- "Simulation_Stats_Homophilic_Network/plot_data"
...
data_homophily_sweep <- read.csv("Simulation_Stats_Homophilic_Network/homoph_sweep_single_00_03/results_00_03.csv")
...
graph_file <- 'regular-graph-rewired'
...
data_dir <- sprintf("Simulation_Stats_Homophilic_Network/%s/1inf_hom_0.00_0.50_by_0.01_alpha_0.00_1.00_by_0.01", graph_file)
...
for (i in 1:length(centralities)) {
  ...
  output_file <- sprintf("Simulation_Stats_Homophilic_Network/%s/1inf_hom_0.00_0.50_by_0.01_alpha_0.00_1.00_by_0.01_%s.html", graph_file, centrality)
